...
def load_wordembedding_300(data_path, vocab, dataname='WiKi'):  # initial equals Wiki
    fvec = '/home/pengda/GoogleNews-vectors-negative300.bin' # initials without #
    word2id, word_vecs = dict(), dict()
...
def clean_OOV(rela2doc, word2id):
    ...
    for rela, doc in rela2doc.items():
        ...
        doc = doc.split() #  e1 description description description description description e2
        cleaned_doc_id = list()
        cleaned_doc = list()
        for word in doc:
...
def NELL_text_embedding(data_path, dataname='NELL'):
    ...
    with open(data_path + "/rela_document.txt", encoding='utf-8') as f_doc:
        ...
        for num in range(181):
            rela = lines[5 * num].strip().split('###')[0].strip()    # drughassideeffect
            description = lines[5 * num + 1].strip().split('###')[1].strip()
                # This predicate indicates that the use of a drug has a physiologicalCondition as a side-effect.  Uses of this predicate may include both very common side-effects, and rare but reported side-effects.
            description = clean_str(description)
            e1 = lines[5 * num + 2].strip().split('###')[1].strip()  # CNC film rating (France)
            e1 = clean_str(e1)
    ...
    WordMatrix = np.load(data_path +'/WordMatrix_300_NELL.npz')['arr_0']
    word2id = pickle.load(open(data_path + '/word2id_300_NELL.pkl', 'rb'))
    ...
    rela_matrix_NELL = generate_matrix_(WordMatrix, corpus, corpus_tfidf, rela_list)
    print(rela_matrix_NELL)
    rela_matrix_NELL = rela_matrix_NELL * 100
    np.savez(data_path + "/rela_matrix", relaM=rela_matrix_NELL)
